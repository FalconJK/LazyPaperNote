...
def Lazy_runner(modifytext_func, **modifytext_kwargs):
...
def opening_msg():
    print("\n==============================================================\n")
    print(
        """    WELCOME TO LazyPaperNote v1.0.0 ^_^
    歡迎使用 LazyPaperNote !
    希望這項小工具能提升您的工作效率!
    -------------------------------
    > Author: github@falconjk
    > Github: https://github.com/FalconJK
    ------------------------------- """)
...
def input_question():
    while True:
        ...
        try:
            print(
                "請選擇文章輸出模式 Please select the 'modify mode'(default is 1): \n  1 基本 basic\n  2 列表式 list-mark\n  3 MarkDown\n 3輸入你的選擇(數字)後按下Enter Your Selection: ")
    ...
    while True:
        ...
        try:
            print(
                "請選擇是否清洗文章內emoji符號 Please select the 'keep_emoji' option(default is 1): \n  1 保存 keep it!\n  2 清洗 remove it\n  輸入你的選擇(數字)後按下Enter Your Selection: ")
    ...
    elif modify_mode_num == 3:
        modify_mode = 'MarkDown'
    else:
        modify_mode = 'BASIC'
...
if __name__ == '__main__':
    ...
    try:
        logging.info("LazyPaperNote is running!")
        Lazy_runner(
            modify_en_text, modify_mode=modify_mode, keep_emoji=keep_emoji)
    except KeyboardInterrupt:
        logging.info("LazyPaperNote is Exiting. Wish you a nice day^^~")
        sys.exit(0)
    except:
        unexpected_exit_msg()
        input()
